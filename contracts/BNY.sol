...
contract BNY  {
    ...
    constructor ()  public {
         totalSupply = 762300000 ;
        balanceOf[msg.sender] = 762300000;
        balanceOf[address(0)] = 0;
        emit Transfer(address(0),msg.sender , 762300000);
    ...
    function getInterestrate(uint256 _investment) public returns (uint256 rate) {
        require(_investment < totalSupply,"wrong");

        uint256 totalinvestments = balanceOf[address(0)] * 100000;
        uint256 investmentsprecenteg = totalinvestments / totalSupply;
        uint256 adjustedinterestrate = (100000 - investmentsprecenteg) * interestRate;
        uint256 interestoninvestment = (adjustedinterestrate * _investment) / 1000000000;

        return (interestoninvestment);
    }
    ...
    function investment(uint256 _unlockTime,uint256 _amount,uint term123) public returns (uint256) {
        ...
        uint termAfter = (_unlockTime / 60);
        if((termAfter >= 1) && (termAfter <= 24) && (term123 == 1))
        {
            investmentTerm = "short";
            totalinvestmentafterinterest = _amount + ((getInterestrate(_amount) * termAfter));
        ...
        if((_unlockTime >= 120) && (term123 == 2) && (_amount >= 2500000) && (_unlockTime % 120) == 0){
            ...
            investmentTerm = "mid";
            totalinvestmentafterinterest = _amount + ((getInterestrate(_amount) * termAfter) * multiplicationForMidTerm);
            Investors[investorIndex] = Investment(msg.sender,totalinvestmentafterinterest,block.timestamp + _unlockTime,false,investmentTerm);
            investorIndex ++;
            balanceOf[msg.sender] -= _amount;
            balanceOf[address(0)] += totalinvestmentafterinterest;
        ...
        if((_unlockTime >= 180) && (term123 == 3) && (_amount >= 7500000) && (_unlockTime % 180 == 0)){
            ...
            investmentTerm = "long";
            totalinvestmentafterinterest = _amount + ((getInterestrate(_amount) * termAfter) * multiplicationForLongTerm);
            Investors[investorIndex] = Investment(msg.sender,totalinvestmentafterinterest,block.timestamp + _unlockTime,false,investmentTerm);
            investorIndex ++;
            balanceOf[msg.sender] -= _amount;
            balanceOf[address(0)] += totalinvestmentafterinterest;
    ...
    function releaseInvestment(uint256 investmentId) public returns (bool success) {
        ...
        require(block.timestamp >= Investors[investmentId].investmentuUnlocktime, "Unlock time for the investment did not pass");
    ...
    function passiveIncomeInvestment(uint256 _amount) public returns (uint256 ID2) {
        ...
        uint256 interestOnInvestment =  ((getInterestrate(_amount) * 48) / 10);
        Investors2[investor2Index] = passiveIncome(msg.sender,_amount,interestOnInvestment,block.timestamp ,block.timestamp + (15 * 10),1,false);
        investor2Index ++;
        balanceOf[msg.sender] -= _amount;
        balanceOf[address(0)] += (interestOnInvestment * 10)+ _amount;
        totalSupply -= _amount;
        emit Deposit2(msg.sender, _amount);
        return (investor2Index);

    }
    function getPasiveIncomeDay(uint256 pasiveincomeID) public returns (uint256 ID2) {
    ...
    require(block.timestamp >= Investors2[investmentId2].investmentTimeStamp + (15 * Investors2[investmentId2].day)  , "Unlock time for the investment did not pass");
    require(Investors2[investmentId2].day < 11 , "The investment is already spent");
    ...
    if(Investors2[investmentId2].day == 10)
    {
    ...
    emit Spent(msg.sender, Investors2[investmentId2].dailyPassiveIncome);
     if(block.timestamp >= Investors2[investmentId2].investmentTimeStamp + (15 * Investors2[investmentId2].day))
     {
    ...
    function () payable external{
        ...
        require(tokensSold < 227700000, "All tokens are sold");
       

        uint256 eth = (msg.value*100000) / ( 1 ether /10);
        uint256 tokens = (eth * tokenPrice)/100000 ;
        uint256 bounosTokens = getDiscountOnBuy(tokens);

        require(bounosTokens + tokens <= 227700000 - tokensSold, "All tokens are sold");
